...
@if(@count($clients))
          ...
          <div class="leads-table">
            <div class="search-block">
              <form action="/manager/clients" method="GET">
            ...
            <div class="leads-thead">
              ...
              <div class="leads-col @if($sort=='fio') active @endif" data-sort="fio" data-dir="@if($sort == 'fio' && $dir == 'asc') desc @else asc @endif">
                Ф.И.О.
                <img src="/images/down.png" class="al-rotate @if($sort == 'fio' && $dir == 'desc') down @else up @endif @if($sort != 'fio') hide @endif"/>
              </div>
              ...
              <div class="leads-col  action-col @if($sort=='created_at') active @endif" data-sort="created_at"data-dir="@if($sort == 'created_at' && $dir == 'asc') desc @else asc @endif">
                Дата создания 
            ...
            <div class="leads-tbody">
              ...
              <div class="leads-main-row">
                <div class="leads-row" data-id="{{$item->id}}" data-name="{{$item->name}}" data-fio="{{$item->fio}}" data-email="{{$item->email}}" data-phone="{{$item->phone}}" data-cities='{{$item->city}}' data-comment="{{$item->comment}}" data-details="{{$item->details}}">
...
@endif
            <div class="leads-form">
              <h2>Данные</h2>
              @if(@$old['id'])
              <form action="/manager/clients/{{$old['id']}}/update" method="POST" id="add_upd_form">
              @else
              <form action="/manager/clients/add" method="POST" id="add_upd_form">
              @endif
                ...
                <div class="input-block">
                  <label>Ф.И.О.</label>
                  <input type="text" name="fio" value="{{@$old['fio']}}" id="lead_fio" required @if(isset($errors['fio'])) class="data-error" @endif/>
                  @if(isset($errors['fio']))
                ...
                <div class="input-block">
                  ...
                  @if(@$old['id'])
                  <button class="submit" id="submit_button">Обновить клиента</button>
                  @else
                  <button class="submit" id="submit_button">Добавить в клиенты</button>
                  @endif
                </div>
              </form>
              <hr>
              <a href="#" class="al-clients-details-button al-details hide">Детальная информация</a>
            </div>
          </div>
        </div>
      </div>
