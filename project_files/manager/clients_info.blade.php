  ...
  <section id="result-search-list" class="al-client-info">
    <div class="container-fluid container-fluid-base">
      <div class="row no-gutters">
        <div 
          class="container container-base container-search-result manager-search client-info-container"
        >
          <h1 class="title-search-result">
            {{ $data['seo']->h1_title }}
          </h1>
          <div class="content-block content-info">
            <div class="al-overlay hide"></div>
            <span class="h2">Название</span>
            <div class="input-group">
              <div class="input-block">
                <label>Название компании</label>
                <input type="text" name="name" value="{{$client->name}}" class="full_name client-info-name"/>
                <span class="error name-error"></span>
              </div>
              <div class="input-block">
                <button class="btn crm-button name-save hide" data-id="{{$client->id}}">Сохранить</button>
              </div>
            </div>
              ...
              <div class="conacts-list">
                ...
                <div class="contact-item">
                  <form class="contact-item-form">
                    ...
                    <button class="del-contact-item">×</button>
                  </form>
                </div>
                @endforeach
              </div>
              <button class="crm-button contacts-add">Добавить контакт</button>
              <!-- overlay was here -->
              <form class="contact-item-form-add hide" id="new_contact">
            ...
            <div class="contacts-block">
              <span class="h2">Компании</span>
              <div class="details-list">
                @foreach($details as $key => $item)
                <div class="details-item clearfix">
                  <span class="details-name">
                  @if($item->client_type_id != 3)
                    {{$item->name}}
                  @else
                    {{$item->fio_full}}
                  @endif
                  </span>
                  @if(!$item->contracts_exist)
                  <span class="details-delete">
                    <a class="details-del" data-id="{{$item->id}}" data-client_id="{{$client->id}}">
                  ...
                  @endif
                  <span class="details-edit">
                    <a class="edit" href="/manager/clients/{{$item->client_id}}/details/{{$item->id}}">
                ...
                @endforeach
                <a href="/manager/clients/{{$client->id}}/details" class="crm-button">Добавить компанию</a>
              </div>
            </div>
            <div class="contacts-block">
              <span class="h2">Договоры</span>
                    ...
                    @foreach($item->acts as $key2 => $act)
                      <div class="contract-planes" id="act-{{$item->id}}_{{$act->id}}" data-contract_id="{{$item->id}}" data-act_id="{{$act->id}}">
                        ...
                        <div class="contract-planes-buttons clearfix @if($act->file) hide @endif" id="cpb_{{$item->id}}_{{$act->id}}">
                          ...
                          <a class="dwn-bill crm-button crm-button-default">
                            Скачать счёт
                            <ul>
                            @foreach($act->month as $key3 => $month)
                              <li data-date="{{$key3}}">{{$month}}</li>
                            @endforeach
                            </ul>
                          </a>
                          @endif
                          ...
                          <div class="selection-items-table @if(!@count($act->boards)) hide @endif">
                            ...
                            <div class="selection-items-body" data-contract_id="{{$item->id}}" data-act_id="{{$act->id}}">
                              @if(@count($act->boards))
                                ...
                                $act_id = $act->id;
                                @endphp
                                @include('front.manager.clients_contract_board_item')
                              @endif
                            </div>
                          </div>   
                        </div>
                        <div class="contract-services-list @if($act->file) hide @endif">
                          <div class="contract-services-list-contrainer @if(!count($act->services)) hide @endif" id="services-{{$item->id}}-{{$act->id}}">
                            <span>Дополнительные услуги</span>
                            <div class="contract-services-list-items">
                              <div class="contract-service-header">
                                <div class="service-items-col">Назвнание</div>
                                <div class="service-items-col">Количество</div>
                                <div class="service-items-col nowrap">Цена без НДС</div>
                                <div class="service-items-col">Итого</div>
                                <div class="service-items-col">&nbsp;</div>
                              </div>
                              @foreach($act->services as $k => $service)
                              <div class="contract-service-line">
                                <div class="service-items-col">{{$service->name}}</div>
                                <div class="service-items-col">{{$service->count}}</div>
                                <div class="service-items-col">{{$service->price}}</div>
                                <div class="service-items-col">{{$service->sum}}</div>
                                <div class="service-items-col">
                                  <a class="service-item-del" data-contract_id="{{$item->id}}" data-act_id="{{$act->id}}" data-id="{{$service->id}}">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                      <path fill-rule="evenodd" clip-rule="evenodd" d="M2.25 6C2.25 5.58579 2.58579 5.25 3 5.25H21C21.4142 5.25 21.75 5.58579 21.75 6C21.75 6.41421 21.4142 6.75 21 6.75H3C2.58579 6.75 2.25 6.41421 2.25 6Z" fill="#FC6B40"/>
                                      <path fill-rule="evenodd" clip-rule="evenodd" d="M10 2.75C9.66848 2.75 9.35054 2.8817 9.11612 3.11612C8.8817 3.35054 8.75 3.66848 8.75 4V5.25H15.25V4C15.25 3.66848 15.1183 3.35054 14.8839 3.11612C14.6495 2.8817 14.3315 2.75 14 2.75H10ZM16.75 5.25V4C16.75 3.27065 16.4603 2.57118 15.9445 2.05546C15.4288 1.53973 14.7293 1.25 14 1.25H10C9.27065 1.25 8.57118 1.53973 8.05546 2.05546C7.53973 2.57118 7.25 3.27065 7.25 4V5.25H5C4.58579 5.25 4.25 5.58579 4.25 6V20C4.25 20.7293 4.53973 21.4288 5.05546 21.9445C5.57118 22.4603 6.27065 22.75 7 22.75H17C17.7293 22.75 18.4288 22.4603 18.9445 21.9445C19.4603 21.4288 19.75 20.7293 19.75 20V6C19.75 5.58579 19.4142 5.25 19 5.25H16.75ZM5.75 6.75V20C5.75 20.3315 5.8817 20.6495 6.11612 20.8839C6.35054 21.1183 6.66848 21.25 7 21.25H17C17.3315 21.25 17.6495 21.1183 17.8839 20.8839C18.1183 20.6495 18.25 20.3315 18.25 20V6.75H5.75Z" fill="#FC6B40"/>
                                      <path fill-rule="evenodd" clip-rule="evenodd" d="M10 10.25C10.4142 10.25 10.75 10.5858 10.75 11V17C10.75 17.4142 10.4142 17.75 10 17.75C9.58579 17.75 9.25 17.4142 9.25 17V11C9.25 10.5858 9.58579 10.25 10 10.25Z" fill="#FC6B40"/>
...
<!-- ACT FINISH -->
              ...
              <a class="crm-button add-contract">Добавить договор</a>
              <form class="contract-form-add hide" id="new_contract">
                <button class="mfp-close">×</button>
                <input type="hidden" name="client_id" value="{{$client->id}}"/>
                <div class="input-block">
                ...
                <div class="input-block">
                  ...
                  @foreach($details as $key => $item)
                    <option value="{{$item->id}}">{{ ($item->client_type_id != 3) ? $item->name_short : $item->fio_full}}</option>
                  @endforeach
                  </select>
                </div>
                <div class="input-block">
            ...
            <div class="selection-list hide">
              ...
              <div class="leads-selections">
                ...
                <a href="/manager/selection/{{$v->id}}?from=client_info" class="ajax-popup-link">
