    ...
    <div class="container-fluid container-fluid-base">
        <div class="row no-gutters navigation-wrapper">
            <div class="container container-base">
                <a class="back" href="/manager/suppliers/{{$supplier->id}}/view">
                ...
                <div class="lead-block-info">
                    <form method="POST" class="client-maindata-form">
                        @csrf
                        <input type="hidden" name="client_logo" value="{{$supplier->logo}}"/>
                        <input type="hidden" value="{{ $supplier->id }}" name="id"/>
                        <h2>@lang('message.information_about_the_company')</h2><br>
                        <div class="field-container">
                                ...
                                @foreach($types as $item)
                                    <div class="supplier-type-item">
                                        <input type="checkbox" name="type_id[]" value="{{$item->id}}"
                                               @if(in_array($item->id, $supplier->has_type_ids)) checked @endif/>{{$item->name}}
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        <div class="field-container">
                            ...
                            <div class="input-block wide-select">
                                ...
                                <select class="js-example-basic-multiple-- tagged-cities" name="cities[]"
                                        multiple="multiple" id="lead_cities">

                                    @foreach($cities as $key => $item)
                                        <option value="{{$item->id}}"
                                                @if(in_array($item->id, $supplier->cities)) selected @endif>{{$item->name}}</option>
                                    @endforeach
                            ...
                            <div class="input-block">
                                ...
                                <input type="text" name="phone" id="lead_phone" value="{{ $supplier->phone }}"/>
                                @if($errors['phone'])
                            ...
                            <div class="input-block mr0">
                                ...
                                <input type="text" name="site" id="lead_site" value="{{ $supplier->site }}"/>
                                @if($errors['site'])
                              ...
                              <div class="input-block">
                                ...
                                <div class="form_toggle">
                                  <div class="form_toggle-item item-1">
                                    <input id="nds-1" type="radio" name="is_nds" value="1" @if($supplier->is_nds == '1') checked @endif>
                                    <label for="nds-1">@lang('message.yes')</label>
                                  </div>
                                  <div class="form_toggle-item item-2">
                                    <input id="nds-0" type="radio" name="is_nds" value="0" @if($supplier->is_nds == '0') checked @endif>
                        ...
                        <div class="hr"></div>
                            <div>
                                <h2>@lang('message.contact_face')</h2><br>
                                <div class="field-container">
                                    <div class="input-block">
                                        <label>@lang('message.contact_face')</label>
                                        <select name="contact_person" class="al-dropdown">
                                            @foreach($contacts as $contact)
                                                <option value="{{$contact->id}}"
                                                        @if($contact->id == $supplier->contact_person) selected @endif>{{$contact->fio}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="input-block">
                                        ...
                                        <select name="channel" class="al-dropdown">
                                            <option value="телеграмм"
                                                    @if( $supplier->channel == 'телеграмм') selected @endif>@lang('message.telegram')</option>
                                            <option value="вайбер"
                                                    @if( $supplier->channel == 'вайбер') selected @endif>@lang('message.viber')</option>
                                            <option value="телефон"
                                                    @if( $supplier->channel == 'телефон') selected @endif>@lang('message.phone_')</option>
                                            <option value="почта"
                                                    @if( $supplier->channel == 'почта') selected @endif>@lang('message.mail')</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        <div class="hr"></div>
                            <div>
                                <h2>@lang('message.comment1')</h2><br>
                                <div class="field-container">
                                    <div class="textarea-block">
                                        <label>@lang('message.note')</label>
                                        <textarea name="comment">{{ $supplier->comment }}</textarea>
                                    </div>
                                </div>
                            </div>
                        <div class="hr"></div>

                        @if(Auth::user()->role_id == 1 || in_array(Auth::user()->id, [207]))
                            <h2>@lang('message.statuses')</h2><br>
                            <div class="tabs-item status-tab" id="t-{{$supplier->id}}-status"
                                 style="background:#fff;padding:20px;">
                                <div class="status-div">
                                    @php
                                        $statusLog = $supplier->statusLog();
                                    @endphp
                                    @if($statusLog && count($statusLog))
                                        @foreach($statusLog as $statusItem)
                                            ...
                                            <hr>
                                        @endforeach
                                        <span>{{date('d.m.Y H:i:s', strtotime($supplier->created_at))}} - <b>{{$statusItem->old_status ? $statusItem->old_status->getTranslatedAttribute('name', \App::getLocale(), 'ru') : ''}}</b></span>
                                        <br>
                                    @else
                                        <span>{{date('d.m.Y H:i:s', strtotime($supplier->created_at))}} - <b>{{$supplier->status ? $supplier->status->getTranslatedAttribute('name', \App::getLocale(), 'ru') : ''}}</b></span>
                                    @endif
                                </div>
                                <div class="status-div">
                                    <input type="hidden" name="old_status_id" value="{{$supplier->status->id}}"/>
                                    <div class="field-container">
                                        <div class="input-block">
                                            <label>@lang('message.status')</label>
                                            <select class="w540 al-dropdown" name="new_status_id"
                                                    data-old_status_id="{{$supplier->status->id}}">
                                                <option value="">-</option>
                                                @foreach($supplier->availableStatuses() as $statusId => $statusName)
                                                    <option value="{{$statusId}}"
                                                            @if($statusId == $supplier->status->id) selected @endif>{{$statusName}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div><br>
                                    <div class="field-container comment-container"
                                         @if($supplier->status_id == 7) style="display: none" @endif>
                                        <div class="input-block">
                            ...
                            <div class="field-container right">
                                <div class="buttons-block">
                                    <a href="/manager/suppliers/{{$supplier->id}}/view"
                                       class="cancel">@lang('message.cancel')</a>
                                    <button class="submit">
                                        @lang('message.save')
                                    </button>
                                </div>
