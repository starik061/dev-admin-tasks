...
<body>
      ...
      <div class="row no-gutters navigation-wrapper">
        ...
        <div class="container  container-base">
          ...
          <div class="client-tab-data-block">
            <form action="{{ route('suppliers.parser.update', ['id' => $supplier->id,'parser_id' => $supplier->firm->id]) }}" method="POST">
              <input type="hidden" value="{{ $supplier->id }}" name="supplier_id"/>
              <input type="hidden" value="{{ $supplier->firm->id }}" name="parser_id"/>
              @csrf
              <div class="suppliers-tabs-header">
                <ul class="tab-navigate">
                  <li class="tab-item active" data-tab="#main-info">@lang('message.basic_information')</li>
                  <li class="tab-item" data-tab="#price-1">@lang('message.parser_tab_price1')</li>
                  <li class="tab-item" data-tab="#price-2">@lang('message.parser_tab_price2')</li>
                  <li class="tab-item" data-tab="#price-3">@lang('message.extra_charge')</li>
                </ul>
              </div>
              <div class="tab-body">
                  ...
                  <div class="info-block">
                    ...
                    <div class="field-container">
                      <div class="input-block wide-select">
                        ...
                        <input type="text" value="{{ $supplier->firm->name }}" name="name"/>
                      </div>
                      <div class="input-block">
                        <div class="switcher-block">
                          <div class="switcher-label">@lang('message.active_parser')</div>
                          <div class="switcher">
                            <input type='checkbox' class='ios8-switch' name="active" value="true" id="active" @if($supplier->firm->active == "true") checked @endif>
                    ...
                    <div class="field-container">
                      <div class="input-block wide-select">
                        ...
                        <input type="text" value="{{ $supplier->firm->descr }}" name="descr"/>
                      </div>
                      <div class="input-block wide-select cities-select">
                        ...
                        @foreach($cities as $key => $item)
                          <option value="{{$item->id}}" @if(in_array($item->id, $supplier->firm->cities)) selected @endif>{{$item->name}}</option>
                        @endforeach
                        </select>
                      </div>
                      <div class="input-block">
                        <label>E-mail</label>
                        <input type="text" value="{{ $supplier->firm->email }}" name="email"/>
                      </div>
                      <div class="input-block">
                        <label>@lang('message.link_to_grid')</label>
                        <input type="text" value="{{ $supplier->firm->link }}" name="link"/>
                      </div>
                    </div>
                    <div class="field-container">
                      <div class="input-block">
                        <label>@lang('message.comment1')</label>
                        <input type="text" value="{{ $supplier->firm->comment }}" name="comment"/>
                      </div>
                    </div>
                    <hr/>
                  </div>
                  ...
                  <div class="info-block">
                    <div class="field-container">
                      <div class="input-block days-block">
                        <label>@lang('message.send_days')</label>
  @php
  $week_days = [
  ...
  @endphp                      
                        <select name="send_days[]" class="send_days" multiple="multiple">
                          ...
                          <option value="{{$k}}" @if(@in_array($k, json_decode($supplier->firm->send_days))) selected @endif>{{$v}}</option>
                          @endforeach
                        </select>
                      </div>
                      <div class="input-block">
  ...
  @endphp
                        <select name="send_week" class="send_week">
                          ...
                          <option value="{{$k}}" @if($k == $supplier->firm->send_week) selected @endif>{{$v}}</option>
                          @endforeach
                        </select>
                      </div>
                      <div class="input-block">
                        <label>@lang('message.last_send')</label>
                        <input type="text" value="{{ $supplier->firm->last_send ? date('d.m.Y',strtotime($supplier->firm->last_send)) : '' }}" name="last_send" disabled/>
                      </div>
                      <div class="input-block">
                        <label>@lang('message.next_send')</label>
                        <input type="text" value="{{ $supplier->firm->next_date ? date('d.m.Y',strtotime($supplier->firm->next_date)) : '' }}" name="next_date"/>
                      </div>
                      <div class="input-block">
                        <label>@lang('message.extra_charge')</label>
                        <input type="text" value="{{ $supplier->firm->markup }}" name="markup"/>
                      </div>
                      <div class="input-block">
                        <label>@lang('message.VAT')</label>
                        <div class="form_toggle">
                          <div class="form_toggle-item item-1">
                            <input id="fid-1" type="radio" name="fix_price" value="1" @if($supplier->firm->fix_price) checked @endif>
                            <label for="fid-1">@lang('message.VAT')</label>
                          </div>
                          <div class="form_toggle-item item-2">
                            <input id="fid-2" type="radio" name="fix_price" value="0" @if(!$supplier->firm->fix_price) checked @endif>
                      ...
                      <div class="input-block">
                        <div class="switcher-block">
                          ...
                          <div class="switcher">
                            <input type='checkbox' class='ios8-switch' name="hidden_price" value="1" id="hidden_price" @if($supplier->firm->hidden_price == "1") checked @endif>
                  ...
                  <div class="field-container">
                    <div class="input-block days-block employment-text">
                      ...
                      <select name="free_text[]" multiple="multiple" class="free_text">
                        ...
                        <option value="{{$k}}" @if(in_array($k, $employmentFree)) selected @endif>{{$v}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="input-block days-block employment-text">
                      ...
                      <select name="free_color[]" multiple="multiple" class="free_color">
                        ...
                        <option value="{{$k}}" @if(in_array($k, $colorsFree)) selected @endif>{{$v}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="input-block days-block employment-text">
                      ...
                      <select name="reserve_text[]" multiple="multiple" class="reserve_text">
                        ...
                        <option value="{{$k}}" @if(in_array($k, $employmentReserve)) selected @endif>{{$v}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="input-block days-block employment-text">
                      ...
                      <select name="reserve_color[]" multiple="multiple" class="reserve_color">
                        @foreach($colors as $k => $v)
                        <option value="{{$k}}" @if(in_array($k, $colorsReserve)) selected @endif>{{$v}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="input-block days-block employment-text">
                      ...
                      <select name="busy_text[]" multiple="multiple" class="busy_text">
                        ...
                        <option value="{{$k}}" @if(in_array($k, $employmentBusy)) selected @endif>{{$v}}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="input-block days-block employment-text">
                      ...
                      <select name="busy_color[]" multiple="multiple" class="busy_color">
                        @foreach($colors as $k => $v)
                        <option value="{{$k}}" @if(in_array($k, $colorsBusy)) selected @endif>{{$v}}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
                </div>
                <div class="tab-content hide" id="price-1">
                  @if($types)
                  <div class="info-block">
                    ...
                    <div class="field-container">
                      ...
                      <div class="input-block">
                        <label>{{$title}}</label>
                        <input name="prices[{{$type}}]" value="{{ $supplier->firm->prices[$type] }}">
                      </div>
                      @endforeach
                    </div>
                  </div>
                ...
                <div class="tab-content hide" id="price-2">
                  @if(count($types))
                  <div class="info-block">
                    ...
                    <div class="field-container">
                      ...
                      <div class="input-block">
                        <label>{{$title}}</label>
                        <input name="prices_enrty[{{$type}}]" value="{{ $supplier->firm->prices_enrty[$type] }}">
                      </div>
                      @endforeach
                    </div>
                  </div>
                ...
                <div class="tab-content hide" id="price-3">
                  @if(count($types))
                  <div class="info-block">
                    ...
                    <div class="field-container">
                      ...
                      <div class="input-block">
                        <label>{{$title}}</label>
                        <input name="prices_markup[{{$type}}]" value="{{ $supplier->firm->prices_markup[$type] }}">
                      </div>
                      @endforeach
                    </div>
                  </div>
