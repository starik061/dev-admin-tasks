<div class="popup-form">
  <div class='popup-form-header'>
    <span>@lang('message.add_signer2')</span>
    <span class="close">
  ...
  <div class="popup-form-body">
    <form method="POST" action="/manager/suppliers/{{ $supplier->id }}/companies/{{$company->id}}/signers/add" id="signer-form">
      <input type="hidden" name="client_id" value="{{ $supplier->id }}"/>
      <input type="hidden" name="detail_id" value="{{ $company->id }}"/>
      ...
      <div class="field-container">
        <div class="input-block">
          ...
          <input type="text" name="name_r" value="" required/>
          <a class="ncl-2">@lang('message.receive_in_the_genitive_case')</a>
        </div>
        <div class="input-block">
          <label>@lang('message.post_r')</label>
          <input type="text" name="post_r" value="" required/>
        </div> 
      </div>
      <div class="field-container">
        <div class="input-block">
          <label>@lang('message.active_from')</label>
          <input type="text" name="started_at" value="{{ date('d.m.Y') }}" class="datepicker" required>
        </div>
      </div>
