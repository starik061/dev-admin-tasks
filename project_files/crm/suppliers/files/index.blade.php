                ...
                <div class="client-navigation">
                    <div class="favorite-viewed-tab">
                        ...
                        @if(!in_array(Auth::user()->id, [273,279]))
                            <a href="/manager/suppliers/{{ $supplier->id }}/companies" id="company-tab">@lang('message.companies') @if($data['details'])({{$data['details']}})@endif</a>
                        @endif
                        ...
                        @if(in_array(Auth::user()->role_id, [1,2]))
                            <a href="/manager/suppliers/{{ $supplier->id }}/files"  class="active">@lang('message.files')</a>
                        @endif
                    </div>
                </div>
                --}}
                <div class="client-tab-data-block">
                    <div class="client-contacts-header mb20">
                        <div class="left-block">
                            <label for="import_setka">  </label>
                            <input type="file" name="import_setka" id="import_setka"/>
                        </div>
                        <div class="right-block">
                            <a href="/manager/suppliers/{{ $supplier->id }}/files/add" name="import_file_setka" id="import_file_setka" class="btn submit">@lang('message.add_files')</a>
                        </div>
                    </div>
                    {{--<hr class="mtb20"/>--}}
                    @if(count($loadfiles))
                            ...
                            <div class="data-table">
                                ...
                                <div class="data-thead">
                                    <div class="data-tr">
                                        ...
                                        <div class="data-td">@lang('message.creation_date')</div>
                                ...
                                <div class="data-tbody">
                                    @foreach($loadfiles as $loadfile)
                                        <div class="data-tr" id="company_tr_{{$loadfile->id}}">
                                            <div class="data-td td-name"><a href="{{$loadfile->url}}" target=”_blank”>{{ $loadfile->filename}}</a></div>
                                            <div class="data-td">{{ $loadfile->ndate}}</div>
                                            <div class="data-td action-col"></div>
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    @endif
                    <div class="no-company @if(count($loadfiles)) hide @endif">
