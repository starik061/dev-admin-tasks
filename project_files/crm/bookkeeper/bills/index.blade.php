              ...
              <form action="{{ route('bookkeeper.bills') }}" method="GET" id="bookkeeper-bill-search">
                <div class="field-container">
                  ...
                  <div class="input-block small-input-block">
                    ...
                    <select name="paid" class="bills-status-select short_select">
                    ...
                    <label class="checkbox-label">
                      <input type="checkbox" value="1" name="with_deleted" @if(@$params['with_deleted']) checked @endif><span>&nbsp;@lang('message.show_deleted')</span>
                    </label>
                  </div>
                  <div class="input-block small-input-block">
                    <label>@lang('message.results_period')</label>
                    <select name="report_period" class="report_period-select short_select" >
                  ...
                  <div class="input-block small-input-block">
                    ...
                    <select name="manager_id" class="manager-select short_select" >
                      <option value="">-</option>
                      @foreach($managers as $manager)
                      <option value="{{$manager->id}}" @if($manager->id == @$params['manager_id']) selected @endif>{{$manager->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="input-block small-input-block">
                    <label>@lang('message.recipient_')</label>
                    <select name="our_details_id" class="our_details-select short_select" >
                      <option value="">-</option>
                      @foreach($our_details as $v)
                      <option value="{{$v->id}}" @if($v->id == @$params['our_details_id']) selected @endif>{{$v->name_short}}</option>
                      @endforeach
                    </select>
                  </div>
                  {{--
                  <div class="input-block">
                    <label>@lang('message.year') (период размещения)</label>
                    <select name="year[]" class="bills-year-select" placeholder="Выберите год">
                      <option value="">-</option>
                      @foreach($years as $v)
                  ...
                  <div class="input-block">
                    <label>@lang('message.month2') (период размещения)</label>
                    <select name="month[]" class="bills-month-select" placeholder="Выберите месяц">
                      <option value="">-</option>
                      @foreach($months as $v)
      ...
      <div class="field-container">
        <div class="input-block">
          ...
          <select class="status-select" name="status" id="status-select">
            ...
            <option value="{{$status->id}}" data-color="{{$status->color}}" data-background="{{$status->background}}">{{$status->name}}</option>
            @endforeach
          </select>
        </div>
      </div>
...
if(window.location.hash.length){
  //$(window.location.hash).magnificPopup('open');
