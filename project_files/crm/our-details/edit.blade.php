@include('add.head')
<body>
  @include('add.header')
  @include('add.menu')
  @include('add.bread')

<section id="result-search-list" class="al-client-info">
  <div class="container-fluid container-fluid-base">
    <div class="row no-gutters">
      <div class="container container-base container-search-result manager-search our-details">
        <div class="favorite-viewed-tab system-info-tabs">
          @include('front.crm.settings-menu')
        </div>
        <h1 class="title-search-result">
            {{ $data['seo']->h1_title }}
        </h1>
        <div class="content-block content-our-details">
          <form method="POST" enctype="multipart/form-data">
            <div class='input-group'>
              <div class="input-block">
                <label>@lang('message.company_name2')</label>
                <input type="text" name="name" value="{{$details->name}}" required class="full_name--"/>
                @if ($errors->has('name'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('name') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block">
                <label>@lang('message.company_name_short')</label>
                <input type="text" name="name_short" value="{{$details->name_short}}" required class="full_name--"/>
                @if ($errors->has('name_short'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('name_short') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block">
                <label>@lang('message.nds_title')</label>
                <div class="form_toggle">
                  <div class="form_toggle-item item-1">
                    <input id="fid-1" type="radio" name="is_nds" value="1" @if($details->is_nds == '1') checked @endif>
                    <label for="fid-1">@lang('message.yes')</label>
                  </div>
                  <div class="form_toggle-item item-2">
                    <input id="fid-2" type="radio" name="is_nds" value="0" @if($details->is_nds == '0') checked @endif>
                    <label for="fid-2">@lang('message.no')</label>
                  </div>
                </div>
                @if ($errors->has('is_nds'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('is_nds') }}</strong>
                  </span>
                @endif
              </div>
            </div>
            <div class='input-group'>
              <div class="input-block">
                <label>@lang('message.address')</label>
                <input type="text" name="address" value="{{$details->address}}" required class="full_name"/>
                @if ($errors->has('address'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('address') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block mr0">
                <label>@lang('message.phone')</label>
                <input type="text" name="phone" value="{{$details->phone}}"/>
                @if ($errors->has('phone'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('phone') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block" style="margin-right: 15px">
                <label>@lang('message.post_address')</label>
                <input type="text" name="post_address" value="{{$details->post_address}}" required class="full_name"/>
                @if ($errors->has('post_address'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('post_address') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block mr0">
                <label>@lang('message.base_city')</label>
                <input type="text" name="city_of_docs" value="{{$details->city_of_docs}}"/>
                @if ($errors->has('city_of_docs'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('city_of_docs') }}</strong>
                  </span>
                @endif
              </div>
            </div>
            <div class='input-group'>
              <div class="input-block">
                <label>@lang('message.edrpou')</label>
                <input type="text" name="edrpou" value="{{$details->edrpou}}" />
                @if ($errors->has('edrpou'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('edrpou') }}</strong>
                  </span>
                @endif
              </div>
              {{-- Убрать для НЕ НДС --}}
              <div class="input-block no-nds-hide @if($details->is_nds == '0') hide @endif">
                <label>@lang('message.inn2')</label>
                <input type="text" name="inn" value="{{$details->inn}}" />
                @if ($errors->has('inn'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('inn') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block">
                <label>@lang('message.income_limit_without_nds')</label>
                <input type="text" name="income_limit" value="{{$details->income_limit}}" required/>
                @if ($errors->has('income_limit'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('income_limit') }}</strong>
                  </span>
                @endif
              </div>
              {{-- Убрать для НЕ НДС --}}
              {{--
              <div class="input-block no-nds-hide @if($details->is_nds == '0') hide @endif">
                <label>@lang('message.vat_number')</label>
                <input type="text" name="nds" value="{{$details->nds}}" />
                @if ($errors->has('nds'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('nds') }}</strong>
                  </span>
                @endif
              </div>
              --}}
              <div class="input-block  mr0">
                <label>@lang('message.tax_status')</label>
                <input type="text" name="status_tax" value="{{$details->status_tax}}" required class="full_name "/>
                @if ($errors->has('status_tax'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('status_tax') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block">
                <label>@lang('message.based_on')</label>
                <input type="text" name="based_on" value="{{$details->based_on}}" required class="full_name "/>
                @if ($errors->has('based_on'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('based_on') }}</strong>
                  </span>
                @endif
              </div>
            </div>
            <div class='input-group'>
              <div class="input-block">
                <label>@lang('message.signer')</label>
                <input type="text" name="signer" value="{{$details->signer}}" required />
                @if ($errors->has('signer'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('signer') }}</strong>
                  </span>
                @endif
              </div>
              <div class="input-block">
                <label>@lang('message.bill_number_start')</label>
                <input type="text" name="first_bill_number" value="{{$details->first_bill_number}}" required />
                @if ($errors->has('first_bill_number'))
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('first_bill_number') }}</strong>
                  </span>
                @endif
              </div>
            </div>
            <div class='input-group bank-details-block'>
              @foreach($details->banks as $k => $bank)
                <div class="field-container">
                  <div class="input-block">
                    <label>@lang('message.bank_name')</label>
                    <input type="text" name="bank_name[{{$bank->id}}]" value="{{$bank->name}}" required/>
                  </div>
                  <div class="input-block">
                    <label>IBAN</label>
                    <input type="text" name="iban[{{$bank->id}}]" value="{{$bank->iban}}" required/>
                  </div>
                  @if(!$k)
                  <div class="input-block mr0">
                    <label>@lang('message.card_number')</label>
                    <input type="text" name="card_number" value="{{$details->card_number}}"/>
                  </div>
                  @else
                  <div class="input-block mr0" style="width: 290px;"></div>
                  @endif
                  <label for="for_new_{{$bank->id}}" class="use-for-new-clients">
                    <input type="checkbox" name="for_new[{{$bank->id}}]" id="for_new_{{$bank->id}}" value="1" @if($bank->use_for_new_clients) checked @endif />
                    Для новых клиентов
                  </label>
                </div>
              @endforeach
              <span class="crm-button add-bank-account pointer">
                Добавить банковский счет
              </span>
            </div>
            {{--
            <div class='input-group'>
              <div class="input-block">
                <label>Печать и подпись</label>
                <input type="file" name="stamp_file" value=""/>
              </div>
              <div class="input-block mr0">
                @if($details->stamp)
                <img src="data:image/png;base64, {{$details->stamp}}"/>
                @endif
              </div>
            </div>
            --}}
            <div class="buttons-block">
              <button class="crm-button">@lang('message.save')</button>
            </div>
          </form>
        </div>
      </div>
    </div>
  </div>
</section>
@include('add.footer')

<script>
var main_url = '/manager/detials';
</script>

@include('front.crm.scripts')
<style>
.field-container{
    display: flex;
    justify-content: space-between;
    position: relative;
}
.bank-details-del{
  position: relative;
  top: 7px;
  right: 0;
}
.use-for-new-clients{
  position:absolute;
  right: -165px;
  top: 35px;
}
.bank-details-block .input-block input[type=text]{
  width: 290px;
}
</style>
<script>
  const bankRow = `
    <div class="field-container">
      <div class="input-block ">
        <input type="text" name="bank_name[####]" value="" required/>
      </div>
      <div class="input-block">
        <input type="text" name="iban[####]" value="" required/>
      </div>
      <div class="input-block mr0" style="width: 340px;">
        <span class="bank-details-del">
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M1.6875 4.5C1.6875 4.18934 1.93934 3.9375 2.25 3.9375H15.75C16.0607 3.9375 16.3125 4.18934 16.3125 4.5C16.3125 4.81066 16.0607 5.0625 15.75 5.0625H2.25C1.93934 5.0625 1.6875 4.81066 1.6875 4.5Z" fill="#FC6B40"></path>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M7.5 2.0625C7.25136 2.0625 7.0129 2.16127 6.83709 2.33709C6.66127 2.5129 6.5625 2.75136 6.5625 3V3.9375H11.4375V3C11.4375 2.75136 11.3387 2.5129 11.1629 2.33709C10.9871 2.16127 10.7486 2.0625 10.5 2.0625H7.5ZM12.5625 3.9375V3C12.5625 2.45299 12.3452 1.92839 11.9584 1.54159C11.5716 1.1548 11.047 0.9375 10.5 0.9375H7.5C6.95299 0.9375 6.42839 1.1548 6.04159 1.54159C5.6548 1.92839 5.4375 2.45299 5.4375 3V3.9375H3.75C3.43934 3.9375 3.1875 4.18934 3.1875 4.5V15C3.1875 15.547 3.4048 16.0716 3.79159 16.4584C4.17839 16.8452 4.70299 17.0625 5.25 17.0625H12.75C13.297 17.0625 13.8216 16.8452 14.2084 16.4584C14.5952 16.0716 14.8125 15.547 14.8125 15V4.5C14.8125 4.18934 14.5607 3.9375 14.25 3.9375H12.5625ZM4.3125 5.0625V15C4.3125 15.2486 4.41127 15.4871 4.58709 15.6629C4.7629 15.8387 5.00136 15.9375 5.25 15.9375H12.75C12.9986 15.9375 13.2371 15.8387 13.4129 15.6629C13.5887 15.4871 13.6875 15.2486 13.6875 15V5.0625H4.3125Z" fill="#FC6B40"></path>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M7.5 7.6875C7.81066 7.6875 8.0625 7.93934 8.0625 8.25V12.75C8.0625 13.0607 7.81066 13.3125 7.5 13.3125C7.18934 13.3125 6.9375 13.0607 6.9375 12.75V8.25C6.9375 7.93934 7.18934 7.6875 7.5 7.6875Z" fill="#FC6B40"></path>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M10.5 7.6875C10.8107 7.6875 11.0625 7.93934 11.0625 8.25V12.75C11.0625 13.0607 10.8107 13.3125 10.5 13.3125C10.1893 13.3125 9.9375 13.0607 9.9375 12.75V8.25C9.9375 7.93934 10.1893 7.6875 10.5 7.6875Z" fill="#FC6B40"></path>
          </svg>
        </span>
      </div>
      <label for="for_new_####" class="use-for-new-clients">
        <input type="checkbox" name="for_new[####]" id="for_new_####" value="1"/>
        Для новых клиентов
      </label>
    </div>
  `;
  let lastBankDetailsId = {{$lastBankDetailsId}};
  $(document).ready(function(){
    $('.add-bank-account').click(function(){
      lastBankDetailsId++;
      $(this).before(bankRow.split('####').join(lastBankDetailsId));
    });
    $(document).on('click', '.bank-details-del', function(){
      let _this = this;
      createConfirm('Удалить', 'Вы хотите удалить данный банковский счет?', function(){
        $(_this).closest('.field-container').remove();
      })
    })
    $(document).on('click', '[name^="for_new["]', function(){
      $('[name^="for_new["]').prop('checked', false);
      $(this).prop('checked', true);
    })
  })
</script>
