<tr>
    <td>
        <label>
            {{$row ? $row->id : $lastId}}
            <input type="hidden" name="id[]" value="{{$row ? $row->id : $lastId}}"/>
        </label>
    </td>
    <td>
        <label class="days-label">
            <input type="checkbox" name="active[{{$row ? $row->id : $lastId}}]" {!! $row->active ? "checked" : '' !!}>
        </label>
    </td>
    <td>
        <input type="text" name="date[{{$row ? $row->id : $lastId}}]" class="datepicker" value="{{$row->date ?? ''}}" id="d-{{$row ? $row->id : $lastId}}]"/>
        @foreach($weekDays as $day => $dayName)
            <label class="days-label">
                <input type="checkbox" name="days_of_week[{{$row ? $row->id : $lastId}}][]" value="{{$day}}" class='' @if($row && in_array($day, (array)json_decode($row->days_of_week))) checked @endif/>{{$dayName}}
            </label><br>
        @endforeach
    </td>
    <td nowrap="">

            &nbsp;&nbsp;С: <input type="text" name="time_from[{{$row ? $row->id : $lastId}}]" value="{{$row->time_from ?? ''}}" required/>

        <br>

            По:  <input type="text" name="time_to[{{$row ? $row->id : $lastId}}]" value="{{$row->time_to ?? ''}}" required/>

    </td>
    <td>
        <a class="delete-schedule" data-id="{{$row ? $row->id : $lastId}}">
