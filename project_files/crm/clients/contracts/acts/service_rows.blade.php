...
<div class="services-table-tr services-counter" id="service-{{$service->id}}">
  <div class="services-td services-td-name">
    {{ ($service->service->id == 11) ? $service->name : $service->service->name . " " . $service->name }}
    ({{$service->by_supplier ? __('message.performed_by_the_owner') : $service->supplier->name}})
    @if($service->supplier->id == 0)
      <a class="service-detailing pointer"
         data-client_id="{{$client->id}}"
         data-contract_id="{{$contract->id}}"
         data-act_id="{{$act->id}}"
         data-service_id="{{$service->id}}"
      >
        @lang('message.detailing')
      </a>
    @endif
  </div>
  <div class="services-td services-td-count align-right">
    <input class="service-count new-service-input"
           value="{{ $service->count }}"
           data-client_id="{{$client->id}}"
           data-contract_id="{{$contract->id}}"
           data-act_id="{{$act->id}}"
  ...
  <div class="services-td services-td-price-in align-right">
    <input class="service-price-in new-service-input"
           value="{{ $service->price_in }}"
           data-client_id="{{$client->id}}"
           data-contract_id="{{$contract->id}}"
           data-act_id="{{$act->id}}"
           data-service_id="{{$service->id}}"/>
  ...
  <div class="services-td services-td-price align-right">
    <input class="service-price new-service-input"
           value="{{ $service->price }}"
           data-client_id="{{$client->id}}"
           data-contract_id="{{$contract->id}}"
           data-act_id="{{$act->id}}"
