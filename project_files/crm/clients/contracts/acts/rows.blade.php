@php
  ...
  //$paid_periods = $act->paid_periods;
  //$disabledPeriods = end($paid_periods);
  $disabledPeriods = $act->lastPaidPeriod();
  if(in_array(Auth::user()->id,[202,248])){
...
<div class="boards-table-tr" id="board-tr-{{$contract->id}}-{{$act->id}}-{{$board->id}}-{{$board->contract_board_id}}">
  ...
  <div class="boards-td boards-td-period col-date-from-to">
    <input class="date_from checkable"
           ...
           @if($canEdit)
             data-disabled-date="{{$disabledPeriods}}"
           @endif
           value="{{$board->date_to && (explode(".", $board->date_to)[2].'-' .explode(".", $board->date_to)[1].'-'.explode(".", $board->date_to)[0] < date('Y-m-d')) && $contract->number == '-'  ? '' : $board->date_from}}"
           @if(!$added)
           @if(!$canEdit || ($disabledPeriods && $disabledPeriods.'-31' >= (explode(".", $board->date_from)[2].'-' .explode(".", $board->date_from)[1].'-'.explode(".", $board->date_from)[0])) ) disabled @endif
           @endif/>
    -
    <input class="date_to checkable"
           ...
           @if($canEdit)
             data-disabled-date="{{$disabledPeriods}}"
           @endif
           value="{{$board->date_to && (explode(".", $board->date_to)[2].'-' .explode(".", $board->date_to)[1].'-'.explode(".", $board->date_to)[0] < date('Y-m-d')) && $contract->number == '-'  ? '' : $board->date_to}}"
           @if(!$added)
           @if(!$canEdit || ($disabledPeriods && $disabledPeriods.'-31' >= (explode(".", $board->date_to)[2].'-' .explode(".", $board->date_to)[1].'-'.explode(".", $board->date_to)[0])) ) disabled @endif
           @endif/>
  </div>
  ...
  <div class="boards-td boards-td-price-in">
    ...
    <input class="owner-price "
           ...
           value="{{ $board->owner_price ?? $board->start_price ?? 0 }}"
           data-board_id="{{$board->id}}"
           data-contract_id="{{$contract->id}}"
           data-act_id="{{$act->id}}"
           data-client_id="{{$client->id}}"
           data-contract_board_id="{{$board->contract_board_id}}"
           data-selection_item_id="{{$board->selectionItemId}}"
           @if(!$added)
           @if((!$act->can_delete && false) || !$canEdit) disabled @endif
           @endif/>
  </div>
  <div class="boards-td boards-td-price-out">
    ...
    @else
      ...
      <input class="client-price-new checkable"
             ...
             value="{{ $board->client_price ?? $board->price ?? 0 }}"
             data-board_id="{{$board->id}}"
             ...
             @if(!$added)
               @if(!in_array(Auth::user()->id,[202,248]))
                 @if(!$act->can_delete || !$canEdit) disabled @endif
               @endif
             @endif
      />
    @endif
  ...
  <div class="boards-td boards-td-action action-col">
    {{--@if($act->can_delete || $added)--}}
    @if((\App\CrmContractsBoard::canDelete($board->contract_board_id) && in_array(Auth::user()->id, [202,248]) ) || $added || $act->can_delete)
