...
<div class="data-tr" data-contact_id="{{$item->id}}">
  <div class="data-td">
    ...
    {{$item->fio}}
    <span class="callback-info" id="callback-info-{{$item->id}}">
    @if($item->callback[0])
      <br/>Передзвонити {{date('d.m.Y H:i:s', strtotime($item->callback[0]->call_me_datetime))}}
    @endif
    </span>
    </span>
  </div>
  ...
  @if($item->post)
    <span style="@if($item->post->color) color:{{$item->post->color}}; @endif @if($item->post->background) background: {{$item->post->background}}; @endif">&bull;&nbsp;{{$item->post->getTranslatedAttribute('name', \App::getLocale(), 'ru')}}</span>
  @endif
  </div>
  ...
  <div class="data-td action-col align-right">
    {{-- сделать проверку на возможность данной операции --}}
    <a href="/manager/clients/{{$item->client_id}}/contacts/{{$item->id}}/callback/{{$item->callback[0] ? $item->callback[0]->id.'/edit' : 'create'}}" class="client-contact-callback">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M12 6V12L16 14" stroke="#FC6B40" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
    </a>
    <a href="/manager/clients/{{$item->client_id}}/contacts/{{$item->id}}/edit" class="edit">
    ...
    <a class="del_ contact-delete" data-client_id="{{$item->client_id}}" data-id="{{$item->id}}">
      <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
