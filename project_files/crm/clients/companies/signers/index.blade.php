  ...
  <section class="lead-block">
    <div class="container-fluid container-fluid-base">
      <div class="row no-gutters navigation-wrapper">
        ...
        <div class="container  container-base">
          ...
          <div class="client-navigation">
            <div class="favorite-viewed-tab">
              <a href="/manager/clients/{{ $client->id }}/view">@lang('message.information')</a>
              <a href="/manager/clients/{{ $client->id }}/companies" class="active" id="company-tab">@lang('message.companies') @if($data['details'])({{$data['details']}})@endif</a>
              <a href="/manager/clients/{{ $client->id }}/contacts" >@lang('message.contacts') @if($data['contacts'])({{$data['contacts']}})@endif</a>
              <a href="/manager/clients/{{ $client->id }}/contracts" >@lang('message.contracts') @if($data['contracts'])({{$data['contracts']}})@endif</a>
              
              <a href="/manager/clients/{{ $client->id }}/bills" >@lang('message.bills') @if($data['bills'])({{$data['bills']}})@endif</a>
              {{--
              <a href="/manager/clients/{{ $client->id }}/photoreports" >Фотоотчеты@if($data['photoreports']) ({{$data['photoreports']}})@endif</a>
              --}}
              <a href="/manager/clients/{{ $client->id }}/suppliers">@lang('message.owners')</a>
            </div> 
          </div>
          ...
          <div class="client-tab-data-block">
            <div class="client-contacts-header mb20">
              <div class="left-block">
                @if(count($signers) > 1)
                <form action="/manager/clients/{{$client->id}}/companies/{{$companyId}}/signers" method="GET" class="input-block search-form">
		...
		@if(Auth::user()->id != 263)
                <a href="/manager/clients/{{ $client->id }}/companies/{{$companyId}}/signers/add" class="btn submit signer-add-link">@lang('message.add_signer')</a>
		@endif
              </div>
            </div>
            {{--<hr class="mtb20"/>--}}
			...
			@if(Auth::user()->id != 263)
                      ...
                      <div class="sub-action hide">                        
                        <a href="/manager/clients/{{$client->id}}/companies/{{$companyId}}/signers/{{$signer->id}}/edit" class="edit-signer">@lang('message.edit')</a>
                        @if($company->can_delete)
                        <a href="/manager/clients/{{$client->id}}/companies/{{$companyId}}/signers/{{$signer->id}}/delete" class="signer-delete" data-id="{{$signer->id}}">@lang('message.delete')</a>
                        @endif
