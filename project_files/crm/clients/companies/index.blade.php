          ...
          <div class="client-tab-data-block">
            <div class="client-contacts-header mb20">
              <div class="left-block">
                ...
                <form action="/manager/clients/{{$client->id}}/companies" method="GET" class="input-block search-form">
              ...
              <div class="right-block">
                <a href="/manager/clients/{{ $client->id }}/details" class="btn submit">@lang('message.add_company')</a>
              </div>
              @endif
            </div>
            {{--<hr class="mtb20"/>--}}
            ...
            <div class="clients-company-data">
              <div class="data-table">
                ...
                <div class="data-thead">
                  <div class="data-tr">
                    <div class="data-td td-name f7">@lang('message.company_name2')</div>
                    <div class="data-td">@lang('message.vat')</div>
                ...
                <div class="data-tbody">
                  ...
                  <div class="data-tr" id="company_tr_{{$company->id}}" @if($company->active != '1') style="background: /*#fdece2*/ #e8ebed" @endif>
                    <div class="data-td td-name">{{ $company->name ? $company->name : $company->fio_full}}</div>
                    <div class="data-td">{{ $company->client_type_id == '1' ? __('message.VAT') : __('message.notVAT') }}</div>
                    <div class="data-td action-col align-right">
                      ...
                      <div class="sub-action hide">
                        <a href="/manager/clients/{{$client->id}}/companies/{{$company->id}}/view">@lang('message.view')</a>
                        {{--@if(Auth::user()->id != 263)--}}
                        <a href="/manager/clients/{{$client->id}}/details/{{$company->id}}">@lang('message.edit')</a>
                        {{--@endif--}}
                        <a href="/manager/clients/{{$client->id}}/companies/{{$company->id}}/signers">@lang('message.signers')</a>
                        @if($company->active == 1)
                          <a class="deactivate-company pointer" data-company_id="{{$company->id}}" data-active="0">@lang('message.deactivate_company')</a>
                        @else
                          <a class="deactivate-company pointer" data-company_id="{{$company->id}}" data-active="1">@lang('message.activate_company')</a>
                        @endif
                        @if($company->can_delete && in_array(Auth::user()->id, [/*1,202,248*/]))
                        <a href="/manager/clients/{{$client->id}}/companies/delete" class="company-delete" data-id="{{$company->id}}">@lang('message.delete')</a>
                        @endif
            ...
            @endif
            <div class="no-company @if(count($companies)) hide @endif">
...
<div class="default-popup deactivate-client-form hide">
  ...
  <div class="default-popup-body">
    <form method="POST" action="/manager/clients/{{ $client->id }}/company-active" enctype="multipart/form-data" id="deactivate-form">
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '.deactivate-company', function(){
      if($(this).data('active') == 1){
        $.post(
          '/manager/clients/{{ $client->id }}/company-active',
          {
            'client_id':{{ $client->id }},
            'detail_id':$(this).data('company_id'),
        ...
        $('.default-popup-body [name=active]').val($(this).data('active'))
        $('.default-popup-body [name=detail_id]').val($(this).data('company_id'))
