        ...
        <div class="container  container-base">
          <div class="client-details-block">
            <div class="details-header">
              ...
              <a href="/manager/clients/{{$client->id}}/details/{{$company->id}}" class="view-edit-btn">@lang('message.edit')</a>
              @endif
            </div>{{-- dump($company) --}}
            @if(in_array($company->client_type_id, [1,2]))
              @include('front.crm.clients.companies.not-fo')
            @else
              @include('front.crm.clients.companies.fo')
            @endif
          </div>
        </div>
      </div>
