      ...
      <div id="by-contract" class="tab-item active">
        <form class="bill-form-add" id="bill-form-add">
          ...
          <div class="field-container">
            <div class="input-block">
              ...
              <select class="contract-select" name="contract_id" id="contract_id" required>
                ...
                @if(in_array($item->id, $activeContractsIds))
                <option value="{{$item->id}}" data-detail_id="{{$item->detail_id}}">
                  №{{ $item->number }}
                  @if(count($contracts) > 1)
                  {{ $item->our_detail_id == 1 ? "(Прайд)" : ($item->our_detail_id == 2 ? "(Рясной)" : "")}}
                  @endif
                </option>
                @endif
                @endforeach
            ...
            <div class="input-block">
              ...
              <select class="application-select" name="application_id" id="application_id" required>
                ...
                @else
                  ...
                  @if(in_array($item->contract_id, $activeContractsIds))
          ...
          <div class="field-container">
            @php
            $periods = [];
            if(count($contracts) == 1 /*&& count($applications[$contracts[0]->id]) == 1*/ && in_array($contracts[0]->id, $activeContractsIds)){
          ...
          <div class="field-container" style="display: none;">
            <div class="input-block">
              ...
              <div class="radio-button">
                <input type="radio" name="status" value="1" id="sid-1">
                <label for="sid-1">@lang('message.created')</label>
              </div>
              <div class="radio-button">
                <input type="radio" name="status" value="2" id="sid-2"  checked>
          ...
          <div class="field-container view-fix">
            <div class="input-block">
              <label>
                <input type="checkbox" value="1" name="without_pay_date">&nbsp;@lang('message.without_pay_date')
              </label>
            </div>
          </div>
          <div class="field-container">
...
document.billCreating = {
  ...
  @endif
  @if(count($applications) == 1 && count($contracts) == 1)
  application_id: {{$applications[$contracts[0]->id][0]->id}},
  @endif
}
{{--
/*
* {{var_dump($contracts[0])}}
