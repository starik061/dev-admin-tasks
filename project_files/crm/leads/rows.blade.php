...
@foreach($leads as $key => $item)
  ...
  @endphp
  <div class="leads-main-row {{$addClass}}" id="lead-info-{{$item->id}}"
    @if($item->visible != '1') 
    ...
    <div class="leads-row" data-id="{{$item->id}}" data-name="{{$item->name}}" data-email="{{$item->email}}" data-phone="{{$item->phone}}" data-cities='{{$item->city}}' data-comment="{{$item->comment}}">
      ...
      {{--@if(Auth::user()->id != 273)--}}
        <div class="leads-col" style="overflow: hidden; text-overflow: ellipsis; flex: 0 0 155px" title="{{$item->email}}">
            {{$item->email}}
      ...
      <div class="leads-col binotel-info">
        @if($call)
        <img class="al-power-tip2" src="/assets/img/{{ $call['call_type'] == '1' ? 'outcall' : 'incall' }}{{ in_array($call['disposition'],['BUSY','NOANSWER','CANCEL','CONGESTION','CHANUNAVAIL','VM','VM-SUCCESS']) ? '_bad' : '' }}.svg" width="32" title="{{ $call['call_type'] == '1' ? __('message.outcall') : __('message.incall') }} {{ in_array($call['disposition'],['BUSY','NOANSWER','CANCEL','CONGESTION','CHANUNAVAIL','VM','VM-SUCCESS']) ? __('message.unsuccessful') : __('message.successful') }} @lang('message.call')"/>
        @endif
      </div>
      <div class="leads-col binotel-info">
        @if($call)
          ...
          <div class="tabs-item status-tab active" id="t-{{$item->id}}-status" style="background:#fff;padding:20px;">
            ...
            <div class="status-div">
              <form method="POST" action="/manager/leads/{{$item->id}}/change-status" class="lead-change-status-form">
                ...
                <div class="field-container comment-container" @if($item->status_id == 7 || $item->status_id == 11) style="display: none" @endif>
                  <div class="input-block">
                ...
                <div class="field-container call-me-block" @if($item->status->id != 6) style="display:none;" @endif>
                  <div class="info-block">
                    <label class="radio-label @if($item->call_me_datetime) checked @endif"><input type="checkbox" name="call_me" @if($item->call_me_datetime) checked @endif>@lang('message.remind_me_to_contact_the_lead')</label>
                  </div><br>
                ...
                <div class="field-container lead-off-reason fd-column" @if($item->status_id != 7 && $item->status_id != 11) style="display: none" @endif>
                  @php
            ...
            <div class="messages-container">
              ...
              <div class="client-messages-data">
                ...
                <div id="l-{{$item->id}}-m" class="calls tab-item">
                  ...
                  <div class="call-item">
                    <div class="call-ico">
                      <img class="al-power-tip2" src="/assets/img/{{ $call->call_type == '1' ? 'outcall' : 'incall' }}{{ in_array($call->disposition,['BUSY','NOANSWER','CANCEL','CONGESTION','CHANUNAVAIL','VM','VM-SUCCESS']) ? '_bad' : '' }}.svg" title="{{ $call->call_type == '1' ? __('message.outcall') : __('message.incall') }} {{ in_array($call->disposition,['BUSY','NOANSWER','CANCEL','CONGESTION','CHANUNAVAIL','VM','VM-SUCCESS']) ? __('message.unsuccessful') : __('message.successful') }} __('message.call)"/>
                      {{--
                    ...
                    <div>
                      ...
                      @if($callDetails)
                        ...
                        <div style="display:flex; margin-bottom: 15px;">
                          <div class="billsec">
                            @lang('message.expectation'): <b>{{$info->waitsec}}</b> c.<br>
                            @lang('message.duration'): <b>{{$info->billsec}}</b> с.
                          </div>
                      ...
                      @else
                        <div style="display:flex">
                          <div class="billsec">
                            @lang('message.expectation'): <b>{{$call->waitsec}}</b> c.<br>
                            @lang('message.duration'): <b>{{$call->billsec}}</b> с.
                          </div>
                          <div class="link2file">
                            @if($call->link)
                            <a class="binotel-get-call" data-call_id="{{$call->binotel_id}}">@lang('message.audio')</a>
            ...
            <div class="info-table">
              ...
              <div class="info-tr">
                ...
                <div class="info-td">
                  ...
                  &nbsp;<a href="https://www.google.com/maps/search/?api=1&query={{$item->info->map->lat}},{{$item->info->map->lng}}" target="_blank">(@lang('message.on_map')</a>
                  @endif
                </div>
              </div>
              <div class="info-tr">
