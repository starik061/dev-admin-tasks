            ...
            <div class="container container-base container-search-result manager-search mw1460">
                ...
                <div class="content-block clearfix">
                        ...
                        <div class="header-block">
                            ...
                            <div class="right-block">
                                <div class="show-block">
                                    ...
                                    <div class="switcher-block">
                                        <div class="switcher-label">@lang('message.show_rejected')</div>
                                        <div class="switcher">
                                            <input type='checkbox' class='ios8-switch' id='show-status-7'
                                                   @if($show_status_7 == "true") checked @endif>
                                            <label for='show-status-7'>&nbsp;</label>
                                        </div>
                                    </div>
                                ...
                                {{--@if(Auth::user()->id != '273')--}}
                                <div class="add-block">
                                    <a href="{{ route('leads.add') }}" class="new-btn" data-status_id="2">@lang('message.add_lead')</a>
                                </div>
                                {{--@endif--}}
                            </div>
                        </div>
                    ...
                    <div class="leads-thead">
                        <div class="leads-col lead-name leads-new-name-col @if($sort=='created_at') active @endif"
                             data-sort="created_at"
                             data-dir="@if($sort == 'crm_leads.created_at' && $dir == 'asc') desc @else asc @endif">
                            @lang('message.name_creation_date')
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                                 xmlns="http://www.w3.org/2000/svg"
                                 class="@if($sort == 'crm_leads.created_at' && $dir == 'desc') down @endif @if($sort == 'crm_leads.created_at' && $dir == 'asc') up @endif">
                        ...
                        <div class="leads-col leads-status-col">
                            @lang('message.lead_status')
                        </div>
                        <div class="leads-col leads-contacts-col">
                            @lang('message.phone_email')
                        </div>
                        <div class="leads-col">
                            @lang('message.city_')
                        </div>
                            ...
                            @if($currentUser->role_id == 1)
                                ...
                                <div class="row-filter hide">
                                    @foreach($source as $key => $value)
                                        <label for="si_{{$key}}">
                                            <input type="checkbox" name="source[]" id="si_{{$key}}"
                                                   value="{{$value ?: '-'}}"
                                                   @if(isset($params['source']) && in_array($value,$params['source'])) checked @endif/> {{$value ?: '-'}}
                                        </label>
                                    @endforeach
                            ...
                            <div class="leads-col with-filter">
                                ...
                                <div class="row-filter hide">
                                    @foreach($utm_channel as $key => $value)
                                        <label for="ci_{{$key}}">
                                            <input type="checkbox" name="utm_channel[]" id="ci_{{$key}}"
                                                   value="{{$value ?: '-'}}"
                                                   @if(isset($params['utm_channel']) && in_array($value,$params['utm_channel'])) checked @endif/> {{$value ?: '-'}}
                                        </label>
                                    @endforeach
                                    <span class="btn btn-style leads-apply-filter">@lang('message.apply')</span>
                                </div>
                        ...
                        <div class="leads-col binotel-info @if($sort=='updated_at') active @endif"
                             data-sort="updated_at"
                             data-dir="@if($sort == 'crm_leads.updated_at' && $dir == 'asc') desc @else asc @endif">
                            @lang('message.updated_at')
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                                 xmlns="http://www.w3.org/2000/svg"
                                 class="@if($sort == 'crm_leads.updated_at' && $dir == 'desc') down @endif @if($sort == 'crm_leads.updated_at' && $dir == 'asc') up @endif">
                                <path d="M8 4.5H4L5.90476 2.5L8 4.5Z" fill="#3D445C"/>
                                <path d="M8 7.5H4L5.90476 9.5L8 7.5Z" fill="#3D445C"/>
                            </svg>
                        </div>
                    ...
                    @if(@count($leads))
                        <div class="leads-tbody">
                            @include('front.crm.leads.rows-new')
                        </div>
                        @if ($leads->lastPage() > 1)
                            <div class="result-paginator" data-current-page="{{ $leads->currentPage() }}"
                                 data-total-page="{{ $leads->lastPage() }}">
                                <button class="btn btn-style btn-show-more-leads">@lang('message.show_more') <span
                                            class="board count">15</span> @lang('message.show_more_leads')</button>
                                <div class="result-paginator__pages">
                                    <div class='result-paginator__prev'>
                                        <i class="fa fa-arrow-left"></i>
                                        <a href="{!! $leads->previousPageUrl() !!}"><p
                                                    class="result-paginator__prev-title">@lang('message.prev')</p></a>
                                    </div>
                                    {!! $leads->appends($params)->links() !!}
                                    <div class='result-paginator__next'>
                                        <a href="{!! $leads->nextPageUrl() !!}"><p
                                                    class="result-paginator__next-title">@lang('message.next')</p></a>
...
@if(session('allready-accepted'))
    <div class="success-mesage">
        @lang('message.order_from') <b>{{ session('name') }}</b> @lang('message.already_took') {{ session('user_name') }}!
        <div class="notify-close">×</div>
    </div>
@endif
...
<div class="import-by-id-modal hide zi10101">
    <div class="close-outdoor">
    ...
    <h2>@lang('message.add_selections_by_id')</h2>
    <form id="import-by-id-form">
        <div class="entry-header_add_by_id">
            <span>ID</span>
            <span>{{ucfirst(__('message.price'))}}</span>
            <span>{{ucfirst(__('message.code'))}}</span>
            <span>{{ucfirst(__('message.city_'))}}</span>
            <span>{{ucfirst(__('message.side'))}}</span>
            <span>{{ucfirst(__('message.type'))}}</span>
            <span>{{ucfirst(__('message.address_'))}}</span>
            <span>{{ucfirst(__('message.photo'))}}</span>
            <span style="width: 40px;"></span>
        </div>
        <div class="id-entry-group_add_by_id">
            <div class="entry_add_by_id">
                <input type="number" min="1" required class="board-id-input_add_by_id" placeholder="ID" />
                <input type="number" min="1" required class="board-price-input_add_by_id" placeholder="{{ucfirst(__('message.price'))}}" />
                <div class="board-code_add_by_id"></div>
                <div class="board-city_add_by_id"></div>
                <div class="board-side_add_by_id"></div>
                <div class="board-type_add_by_id"></div>
                <div class="board-address_add_by_id"></div>
                <div class="board-photo_add_by_id"></div>
                <button type="button" class="remove-board-entry_add_by_id">🗑</button>
            </div>
        </div>
        <br>
        <button type="button" id="add-new-board-entry_add_by_id" class="btn-style_add_by_id btn-secondary_add_by_id">+ {{ucfirst(__('message.add_more'))}}</button>
        <br><br>
        <button type="submit" class="btn-style_add_by_id">{{ucfirst(__('message.save'))}}</button>
    </form>
</div>
