<div class="leads-row-selects " data-lead_id="{{ $item->id }}">
    @php
            ...
            <div class="tabs-item status-tab active" id="t-{{$item->id}}-status" style="background:#fff;padding:20px;">
                ...
                <div class="status-div">
                    <form method="POST" action="/manager/leads/{{$item->id}}/change-status"
                        ...
                        <input type="hidden" name="selections" value="{{ optional($item->selections)->count() }}" />
                        ...
                        <div class="field-container">
                            <div class="input-block">
                                ...
                                @if($classes)
                                    <div  @if($item->status_id == 7 || $item->status_id == 11) style="display: none" @endif class="class_status_custom_id">
            ...
            <div class="tabs-item message-tab" id="t-{{$item->id}}-messages">
                <div class="messages-container">
                    <div class="messages-tab">
                        <span class="pointer active tab-selector" data-type="message">@lang('message.messages')</span>
...
{{--                                        <div class="link2file" style="margin-left: 10px; cursor: pointer">--}}
{{--                                            <a href="/manager/leads/{{$item->id}}/kcall-transcription/{{$call->id}}"--}}
{{--                                               class="ajax-popup-link" id="act-view-">@lang('message.call_text_version')</a>--}}
{{--                                        </div>--}}
                                    </div>
                                @endforeach
            ...
            <div class="tabs-item message-tab" id="t-{{$item->id}}-comments"
                ...
                <div class="status-div">
                    <form method="POST" action="/manager/leads/{{$item->id}}/update-comment" class="comment-update-form">
                        <div class="field-container comment-container">
                            <div class="input-block">
                                <label>@lang('message.comment1')</label>
                                <textarea name="comment" style="width: 1000px">{{ old('comment', $item->comment) }}</textarea>
                            </div>
                        </div>
                        <div class="align-right">
            ...
            <div class="lead-actions-container">
                ...
                @endcan
                @if(Auth::user()->role_id == 1)
                    <a class="visibility-change pointer" data-visibility="{{$item->visible}}" data-id="{{$item->id}}"
                       id="v-{{$item->id}}">
                        @if($item->visible)
