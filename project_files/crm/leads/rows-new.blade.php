@php
    $codes = [
      ...
      'AUTOMATIC-ONGOING-SUCCESS' => __('message.automatic_ongoing_success_call'),
    ];
@endphp
@foreach($leads as $key => $item)
    @php
        $call = end($item->calls->toArray());
        $keyCallCall = $item->keyCallCalls ? end($item->keyCallCalls->toArray()) : [];

        $lastInteraction = 'call';
        if ($keyCallCall && ($keyCallCall['created_at'] > $call['created_at'])) {
            $lastInteraction = 'key-call';
        }

        $addClass = '';
        if($currentUser->role_id == 2 && ($currentUser->id != $item->user_id && $item->user_id) ){
          $addClass = 'o-4';
        }
    ...
    <div class="leads-main-row {{$addClass}}"
         ...
         @else
             @if($item->status && $item->status->id == 8)
                 style="background: #e7d9fa;"
         @else
             @if($item->from_form && $item->user_id)
                 style="background: #EDF7ED;"
         @else
             @if(($item->from_form && $item->from_form != '0') || ($item->have_supervision != 0 && !$item->user_id) || $item->from_interactive_commercial_offer)
    ...
    >
        <div class="leads-row"
             ...
             @if($item->source == 'Chaport AI Assistant')
                 style="background: url(/assets/img/chaport_gpt.png) 14px 42px no-repeat"
                @endif
        >
            ...
            <div class="leads-col leads-status-col">
                {{optional($item->status)->getTranslatedAttribute('name')}}<br><!--тут проверить eager loading -->
                <svg width="132" height="4" viewBox="0 0 132 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0 2C0 1.17157 0.671573 0.5 1.5 0.5H25.6V3.5H1.5C0.671574 3.5 0 2.82843 0 2Z"
                          fill="{{ ($item->status->id >= 2  && $item->status->id <= 5) || $item->status->id == 6 || in_array($item->status->id, [9,10,11,12,13]) ? '#FC6B40' : '#E8EBF1' }}"/>
                    <rect x="26.6016" y="0.5" width="25.6" height="3"
                          fill="{{ $item->status->id >= 3 && $item->status->id <= 5 || $item->status->id == 10 || in_array($item->status->id, [10,11,12,13]) ? '#FC6B40' : '#E8EBF1' }}"/>
                    <rect x="53.1992" y="0.5" width="25.6" height="3"
                          fill="{{ $item->status->id >= 4 && $item->status->id <= 5 || in_array($item->status->id, [11,12,13]) ? '#FC6B40' : '#E8EBF1' }}"/>
                    <rect x="79.8008" y="0.5" width="25.6" height="3"
                          fill="{{ ($item->status->id == 5) || in_array($item->status->id, [12,13]) ? '#FC6B40' : '#E8EBF1' }}"/>

                    @if($item->status->id == 8)
                        @for($i = 0; $i <= 3; $i++)
                            <path d="M0 2C0 1.17157 0.671573 0.5 1.5 0.5H25.6V3.5H1.5C0.671574 3.5 0 2.82843 0 2Z"
                                  fill="#FC6B40"/>
                            <rect x="26.6016" y="0.5" width="25.6" height="3" fill="#FC6B40"/>
                            <rect x="53.1992" y="0.5" width="25.6" height="3" fill="#FC6B40"/>
                            <rect x="79.8008" y="0.5" width="25.6" height="3" fill="#FC6B40"/>
            ...
            <div class="leads-col binotel-info">
                ...
                @elseif($lastInteraction == 'key-call')
                    <div class="leads-col binotel-info">
                        <img class="al-power-tip2"
                             src="/assets/img/outcall.svg"
                             width="32"
                             title="{{ $codes['AUTOMATIC-ONGOING-SUCCESS'] }}"/>
                    </div>
                @endif
            </div>
            <div class="leads-col binotel-info">
                ...
                @elseif($lastInteraction == 'key-call')
                    <div class="leads-col binotel-info">
                        {{ $codes['AUTOMATIC-ONGOING-SUCCESS'] }}
                    </div>
                @endif
            </div>
            <div class="leads-col binotel-info">
        ...
        @if($currentUser->role_id == 1 || $currentUser->id == $item->user_id || $item->user_id == null || ($currentUser->id == 207 && $item->user_id == 262 && $item->id == 12120))
            <div class="leads-row-selects hide" data-lead_id="{{ $item->id }}">
                ...
                <div class="row-inner-tabs">
                    <div class="row-inner-tabs-nav">
                        {{--@if(Auth::user()->id != 273)--}}
                        <ul>
                            <li data-id="#t-{{$item->id}}-status" class="active">@lang('message.status')</li>
                            <li data-id="#t-{{$item->id}}-selections">@lang('message.selections_')</li>
                            @if($item->messages || count($item->calls) || count($item->keyCallCalls))
                                <li data-id="#t-{{$item->id}}-messages">@lang('message.chatting_history') </li>
                            @endif
                            <li data-id="#t-{{$item->id}}-info">@lang('message.leads_info')</li>
                            @if(Auth::user()->role_id == 1)
                                @if($item->ga_client_id)
                                    <li data-id="#t-{{$item->id}}-ga">Google Analitics</li>
                                @endif
                            @endif
                            <li data-id="#t-{{$item->id}}-comments">@lang('message.comment1')</li>
                        </ul>
                        {{--
                        @else
                        <ul>
                    ...
                    <div class="tabs-content">
                        {{--@if(Auth::user()->id != 273)--}}
                        <div class="tabs-item status-tab active" id="t-{{$item->id}}-status"
                            ...
                            <div class="status-div">
                                ...
                                @if($statusLog && count($statusLog))
                                    @foreach($statusLog as $statusItem)
                                        ...
                                        @if($statusItem->new_status && $statusItem->new_status->id == 8 && $statusItem->key_call)

                                            <div class="call-transcription" style="margin-left: 10px;">
                                                @foreach(json_decode($statusItem->key_call->answers) as $answer)
                                                    <p>- {{ $answer }}</p>
                                                @endforeach
                                            </div>
                                        @endif
                            ...
                            <div class="status-div">
                                <form method="POST" action="/manager/leads/{{$item->id}}/change-status"
                                      class="lead-change-status-form">
                                    ...
                                    <input type="hidden" name="selections"
                                           value="{{ optional($item->selections)->count() }}"/>
                                    <input type="hidden" name="user_role_id"
                                           value="{{ Auth::user() ? Auth::user()->role_id : null }}"/>
                                    <div class="field-container">
                                        <div class="input-block">
                                            <label>@lang('message.status')</label>
                                            <select class="w540 lead_status_id" name="new_status_id"
                                                    data-old_status_id="{{$item->status->id}}">
                                                @foreach($item->availableStatuses() as $statusId => $statusName)
                                                    <option value="{{$statusId}}"
                                                            @if($statusId == $item->status->id) selected @endif>{{$statusName}}</option>
                                                @endforeach
                                            </select>

                                            @if($classes)
                                                <div @if($item->status_id == 7 || $item->status_id == 11) style="display: none"
                                                     @endif class="class_status_custom_id">
                                                    <label>@lang('message.class')</label>
                                                    <select class="w540 class_status_id" name="class_id"
                                                            data-old_class_id="{{$item->class_id}}">
                                                        <option value="" @if($item->class_id == null) selected @endif>-
                                                        </option>
                                                        @foreach($classes as $class)
                                                            <option value="{{$class->id}}"
                                                                    @if($class->id == $item->class_id) selected @endif>{{$class->name}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            @endif
                                    ...
                                    <div class="field-container comment-container"
                                         @if($item->status_id == 7 && $item->status_id == 11) style="display: none" @endif>
                                        <div class="input-block">
                                    ...
                                    <div class="field-container call-me-block"
                                         @if($item->status->id != 6) style="display:none;" @endif>
{{--                                        <div class="info-block">--}}
{{--                                            <label class="radio-label @if($item->call_me_datetime) checked @endif"><input--}}
{{--                                                        type="checkbox" name="call_me"--}}
{{--                                                        @if($item->call_me_datetime) checked @endif>@lang('message.remind_me_to_contact_the_lead')--}}
{{--                                            </label>--}}
{{--                                        </div>--}}
                                        <br>
                                        <div class="input-block  dt"
                                             @if(!$item->call_me_datetime) @endif>
                                            <label>@lang('message.select_date_and_time')</label>
                                            <input type="text" name="call_me_datetime" class="datetime-input"
                                                   placeholder="@lang('message.date_time')" style="width:540px;"
                                                   value="{{ $item->call_me_datetime ? date('d.m.Y H:i', strtotime($item->call_me_datetime)) : '' }}"/>
                                        </div>
                                    </div>
                                    <div class="field-container lead-off-reason fd-column"
                                         @if($item->status_id != 7 && $item->status_id != 11) style="display: none" @endif>
                                        @php
                                        ...
                                        <div class="input-block" style="width:400px;">
                                            ...
                                            @foreach($rejections_reasons as $reason)
                                                <label class="radio-label @if($reason->id == $item->rejection_reason_id) checked @endif"
                                                       @if($reason->rejection_id != $item->rejection_id) style="display: none"
                                                       @endif
                                                    ...
                                                    <input type="radio"
                                                           ...
                                                           value="{{$reason->id}}">
                                                    {{$reason->getTranslatedAttribute('name', 'ua', 'ru')}}
                                                </label>
                                            @endforeach
                                        </div>
                                    </div>
                                    <div class="align-right">
                                        <button class="submit" disabled>@lang('message.change_button')</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        <div class="tabs-item" id="t-{{$item->id}}-selections">
                                        ...
                                        <a href="/manager/selection/{{$v->id}}" class="ajax-popup-link">
                                            <div class="selection-item pointer">
                                                ...
                                                <div class="selection-col" style="flex: 0.3;">
                                                    <span data-url="/manager/selection/{{$v->id}}/delete"
                                                          class="delete-selection pointer">
                                                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none"
                                                             xmlns="http://www.w3.org/2000/svg">
                                                            <path fill-rule="evenodd" clip-rule="evenodd"
                                                                  d="M1.6875 4.5C1.6875 4.18934 1.93934 3.9375 2.25 3.9375H15.75C16.0607 3.9375 16.3125 4.18934 16.3125 4.5C16.3125 4.81066 16.0607 5.0625 15.75 5.0625H2.25C1.93934 5.0625 1.6875 4.81066 1.6875 4.5Z"
                            ...
                            @else
                                <div class="no-selection">
                                    ...
                                    <span class="no-data-title">@lang('message.this_lead_has_no_collections')</span>
                        ...
                        <div class="tabs-item message-tab" id="t-{{$item->id}}-messages">
                            <div class="messages-container">
                                <div class="messages-tab">
                                    ...
                                    <span class="pointer tab-selector"
                                          data-type="keycall-calls">@lang('message.keycall-calls')</span>
                                </div>
                                <div class="client-messages-data">
                                    ...
                                    <div id="l-{{$item->id}}-m" class="calls tab-item">
                                        @if($item->calls)
                                            ...
                                            @foreach($item->calls as $k => $call)
                                                    ...
                                                    <div>
                                                        ...
                                                        @if($callDetails)
                                                            @foreach($callDetails->historyData as $key => $info)
                                                                <div style="display:flex; margin-bottom: 15px;">
                                                                    <div class="billsec">
                                                                        @lang('message.expectation'):
                                                                        <b>{{$info->waitsec}}</b> c.<br>
                                                                    ...
                                                                    @if($call->link && $info->disposition == 'ANSWER' && $call->transcription_status_id === \App\CrmCallsTranscriptionsStatus::TRANSCRIPTION_RECEIVED)
                                                                        @php
                                                                            $callInfo = $info;
                                                                        @endphp
                                                                        <div class="transcription-col">
                                                                            <a class="show-transcription"
                                                                               data-id="transcription-{{$call->id}}">@lang('message.transcription')</a>
                                                                        </div>
                                                                    @endif
                                                                </div>
                                                            @endforeach
                                                        @else
                                                            <div style="display:flex">
                                                                <div class="billsec">
                                                                    @lang('message.expectation'):
                                                                    <b>{{$call->waitsec}}</b> c.<br>
                                                                    @lang('message.duration'): <b>{{$call->billsec}}</b>
                                                                    с.
                                                                </div>
                                                                <div class="link2file">
                                                                    @if($call->link)
                                                ...
                                                @if($call->link && $callInfo->disposition == 'ANSWER' && $call->transcription_status_id === \App\CrmCallsTranscriptionsStatus::TRANSCRIPTION_RECEIVED)
                                    ...
                                    <div id="l-{{$item->id}}-m" class="keycall-calls tab-item">
                                        @if($item->keyCallCalls)
                                            <!-- call_type = 1 - исходящий -->
                                            @foreach($item->keyCallCalls as $k => $call)
                                                <div class="call-item">
                                                    <div class="call-ico">
                                                        <img class="al-power-tip2"
                                                             src="/assets/img/outcall.svg">
                                                    ...
                                                    <div>
                                                        <div style="display:flex">
                                                            <div class="link2file">
                                                                <a class="key_call-get-call"
                                                                   data-call_id="{{$call->call_id}}">@lang('message.audio')</a>
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <div class="call-transcription" style="margin-left: 10px;">
                                                        @foreach(json_decode($call->answers) as $answer)
                                                            <p>- {{ $answer }}</p>
                                                        @endforeach
                                                    </div>
                                                </div>
                        ...
                        <div class="tabs-item message-tab {{--@if(Auth::user()->id == 273) active @endif--}}"
                            ...
                            <div class="info-table">
                                <div class="info-tr">
                                    ...
                                    <div class="info-td">
                                        <a href="{{$item->info->page->url}}"
                                           target="_blank">{{$item->info->page->url}}</a>
                                    </div>
                                </div>
                                <div class="info-tr">
                        ...
                        <div class="tabs-item message-tab" id="t-{{$item->id}}-comments"
                             style="background:#fff;padding:20px;">
                            <div class="status-div">
                                <form method="POST" action="/manager/leads/{{$item->id}}/update-comment"
                                      class="comment-update-form">
                                    <div class="field-container comment-container">
                                        <div class="input-block" style="position: relative; width: 550px;">
                                            <label>@lang('message.comment1')</label>
                                            <textarea name="comment"
                                                      style="width: 100%; padding-bottom: 50px;">{{ old('comment', $item->comment) }}</textarea>
                                            <button type="submit" class="submit" style="
                                                position: absolute;
                                                right: -100px; /* сместили за край */
                                                bottom: 5px;
                                            ">
                                                @lang('message.save')
                                            </button>
                        ...
                        <div class="lead-actions-container">
                            @if((in_array($currentUser->role_id, [2,7]) && $item->user_id == $currentUser->id) || $currentUser->role_id == 1)
                                {{--
                                ...
                                --}}
                                <a class="to-client-action go-to-search-for-contragent" data-id="{{ $item->id }}" data-type="lead">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                              d="M10 3.54163C10.3452 3.54163 10.625 3.82145 10.625 4.16663V15.8333C10.625 16.1785 10.3452 16.4583 10 16.4583C9.65482 16.4583 9.375 16.1785 9.375 15.8333V4.16663C9.375 3.82145 9.65482 3.54163 10 3.54163Z"
                                ...
                                <a class="to-client-action add-by-id" data-lead_id="{{$item->id}}">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                              d="M3.37922 1.71284C3.80899 1.28307 4.39189 1.04163 4.99967 1.04163H12.3419L17.2913 5.99108V16.6666C17.2913 17.2744 17.0499 17.8573 16.6201 18.2871C16.1904 18.7169 15.6075 18.9583 14.9997 18.9583H4.99967C4.39189 18.9583 3.80899 18.7169 3.37922 18.2871C2.94945 17.8573 2.70801 17.2744 2.70801 16.6666V3.33329C2.70801 2.72551 2.94945 2.14261 3.37922 1.71284ZM4.99967 2.29163C4.72341 2.29163 4.45846 2.40137 4.2631 2.59672C4.06775 2.79207 3.95801 3.05703 3.95801 3.33329V16.6666C3.95801 16.9429 4.06775 17.2078 4.2631 17.4032C4.45846 17.5985 4.72341 17.7083 4.99967 17.7083H14.9997C15.2759 17.7083 15.5409 17.5985 15.7362 17.4032C15.9316 17.2078 16.0413 16.9429 16.0413 16.6666V7.29163H11.0413V2.29163H4.99967ZM12.2913 2.75884L15.5741 6.04163H12.2913V2.75884ZM13.333 10.2083H6.66634V11.4583H13.333V10.2083ZM6.66634 13.5416H13.333V14.7916H6.66634V13.5416ZM8.33301 6.87496H6.66634V8.12496H8.33301V6.87496Z"
                                ...
                                @can('view-clients')
                                    <a class="to-client-action" href="/manager/leads/lead2client/{{ $item->id }}">
                                        ...
                                        @lang('message.convert_to_clients')
                                    </a>
                                @endcan
                            @endif
                            @if(!$item->user_id)
                            ...
                            <a class="change-type-action" href="/manager/leads/{{ $item->id }}/change-type">
                                ...
                                @lang('message.convert_to_supplier')
                            </a>
                            @can('view-clients')
